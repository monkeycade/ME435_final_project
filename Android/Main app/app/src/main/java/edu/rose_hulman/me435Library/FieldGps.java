package edu.rose_hulman.me435Library;
...
public class FieldGps implements LocationListener {
     ...
     *
     * @param listener Listener that implements FieldGpsListener that will be called with
     *                 updates.
     */
    public FieldGps(FieldGpsListener listener, InstanceStorage storage) {
        this(listener,
                ROSE_FRONT_CIRCLE_LATITUDE, ROSE_FRONT_CIRCLE_LONGITUDE,
                ROSE_FRONT_ENTRANCE_LATITUDE, ROSE_FRONT_ENTRANCE_LONGITUDE);
        List<Double> data = storage.readColorCoeffList("GPS_SETTING");
        if (data != null && data.size() == 4) {
            mLatitudeOrigin = data.get(0);
            mLongitudeOrigin = data.get(1);
            mLatitudeOnXAxis = data.get(2);
            mLongitudeOnXAxis = data.get(3);
     ...
     * @param listener         Listener that implements FieldGpsListener that will be called with
     *                         updates.
     * @param latitudeOrigin   Latitude of the field origin.
     * @param longitudeOrigin  Longitude of the field origin.
     * @param latitudeOnXAxis  Latitude of any point that is on the X axis.
     * @param longitudeOnXAxis Longitude of any point that is on the X axis.
     */
     ...
     *
     * @param context A context (usually an Activity) used to get the LocationManager.
     */
     ...
     *
     * @param context            A context (usually an Activity) used to get the LocationManager.
     * @param minTime_ms         Time in milliseconds between GPS location updates.
     * @param minDistance_meters Distance in meters between GPS location updates.
     */
     ...
     * the listener.
     *
     * @param location Location to convert and send.
     */
     ...
     *
     * @param angle Original angle that is not normalized.
     ...
     */
    public void setCurrentLocationAsOrigin(InstanceStorage storage) {
        if (mLastGpsLocation != null) {
            setOriginLocation(mLastGpsLocation);
            storage.store("GPS_SETTING", Arrays.asList(mLatitudeOrigin, mLongitudeOrigin, mLatitudeOnXAxis, mLongitudeOnXAxis));
        }
     ...
     */
    public void setCurrentLocationAsLocationOnXAxis(InstanceStorage storage) {
        if (mLastGpsLocation != null) {
