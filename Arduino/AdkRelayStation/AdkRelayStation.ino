...
void setup() {
  ...
  delay(300);
  attachSelectedServosCallback(0b000000);
  delay(200);
  attachSelectedServosCallback(0b111111);
  delay(1000);
  acc.powerOn();
  ...
  //  while(true){
  //    int result[3];
  //    ballStand.determineAllBallColor(result, &lcd);
  //    Serial.print("The ball color is ");
...
void customStringCallbackFromAndroid(String customString) {
  lcd.clear();
  if (customString.substring(0, 16).equalsIgnoreCase("Detect Color at ")) {
    int location = customString.charAt(16) - 48;
    lcd.print("Location ");
    lcd.print(location);
    lcd.print(" color");
    lcd.setCursor(0, LINE_2);
    int reading[5];
    ballStand.getLocationRefelectionReading(location == 1 ? LOCATION_1 : (location == 2 ? LOCATION_2 : LOCATION_3), reading);
    String toRereturn = "location " + String(location);
    if (!ballStand.judgeisball(reading)) {
      toRereturn += " none";
        lcd.print("none");
    } else {
      for (int i = 0; i < 5; i++) {
        toRereturn += " " + String(reading[i]);
        lcd.print(reading[i]);
        lcd.print(" ");
      }
    }
    char buff[toRereturn.length() + 1];
    toRereturn.toCharArray(buff, sizeof(buff));
    acc.write(buff, sizeof(buff));




    //    char temp[] = "Location 1 2";
    //    temp[9] = 49;
    //    temp[11] = 49 + result[0];
    //    acc.write(temp, sizeof(temp));
    //    temp[9] = 50;
    //    temp[11] = 49 + result[1];
    //    acc.write(temp, sizeof(temp));
    //    temp[9] = 51;
